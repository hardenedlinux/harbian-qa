...
func main() {
	...
	for _, frame := range frames {
		if (strings.Contains(frame.Func, *f_funcname) && *f_strict == false) ||
			...
			list[n].Bblock = append(list[n].Bblock, frame.PC)
	...
	for _, e := range list {
		...
		for _, bb := range e.Bblock {
			fmt.Printf("%x, ", bb&0xffffffff)
		}
		fmt.Printf("}\n")
	}
}
// pick out the address of sanitize instrument from vmlinux
func coveredPcs(arch, bin string, rawformat bool) ([]uint64, error) {
	...
	defer cmd.Wait()
	var pcs []uint64
	// scan vmlinux, search sanitize instrument
	// eg. "call <__sanitizer_cov_trace_pc>" on x86
...
// attach source code information to instrumnet address
